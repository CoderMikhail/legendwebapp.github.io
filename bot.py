...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (id INTEGER PRIMARY KEY, username TEXT, balance REAL, leads INTEGER, first_launch BOOLEAN)''')
    conn.commit()
    conn.close()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (id, username, balance, leads, first_launch) VALUES (?, ?, 0, 0, 1)", (user_id, username))
    conn.commit()
    ...
    if first_launch:
        ...
        keyboard = [[InlineKeyboardButton("Панель управления", web_app={'url': 'https://your-web-app-url.com'})]]
